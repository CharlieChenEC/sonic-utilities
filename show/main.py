...
def remote_vni(remote_vtep_ip, count):
    ...
    if (remote_vtep_ip != 'all') and (is_ipaddress(remote_vtep_ip) is False):
...
def brief(ctx):
    ...
    for domain_id in natsorted(domain_keys):
        ...
        if peer_link:
            ...
            if peer_entry:
                peer_link_status = peer_entry.get('oper_status').capitalize() or ''
...
@click.pass_context
def interface(ctx, domain_id, portchannel_name):
...
def unique_ip(ctx):
    ...
    unique_ip_keys = config_db.get_keys('MCLAG_UNIQUEIP_TABLE')
...
def interface_qos(ctx):
    ...
    for k in port_qos_map_keys:
        ...
        # click.echo('  Ingress Rate Limit: {}'.format(ing_scheduler_prof))
        ...
        # click.echo('  Egress Rate Limit : {}'.format(egr_scheduler_prof))
