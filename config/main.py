...
def add_vlan_member(ctx, vid, interface_name, untagged):
    ...
    if not vlan_id_is_valid(vid):
        ctx.fail("Invalid VLAN ID {} (1-4094)".format(vid))

    vlan_name = 'Vlan{}'.format(vid)
    if check_if_vlanid_exist(db, vlan_name) == False:
    ...
    if is_port_vlan_member(db, interface_name, vlan_name):
...
def del_vlan_member(ctx, vid, interface_name):
    ...
    if not is_port_vlan_member(db, interface_name, vlan_name):
...
def breakout(ctx, interface_name, mode, verbose, force_remove_dependencies, load_predefined_config):
    ...
    # Number of interface equals before and after port breakout
    if del_intf_dict.keys() == add_intf_dict.keys():
        for intf in del_intf_dict.keys():
